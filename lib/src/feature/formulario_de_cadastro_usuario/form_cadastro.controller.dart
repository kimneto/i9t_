...
class FormCadastroController extends ValueNotifier<FormCadastroState> {
  FormCadastroController(FormCadastroState value)
      : super(FormCadastroInitial());
  ...
  cadastrarCondutor() async {
    ...
    try {
      if (estaValidado) {
        // value = FormCadastroLoading();
        await condutorService
            .pegaCondutorPorCpf(condutorModel.cpf.toString())
            .then((v) {
          if (v.cpf != null) {
            value = FormCadastroFailure(error: 'Esse CPF já está cadastrado');
          } else {
            condutorService.cadastraCondutor(condutorModel);
            value = FormCadastroSuccess(condutor: condutorModel);
          }
        });
      }
    } catch (e) {
