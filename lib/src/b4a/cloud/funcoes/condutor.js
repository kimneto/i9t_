...
Parse.Cloud.define("cria-condutor", async (req) => {
   ...
   try {
      if (resultCondutor.length <= 0) {
         ...
         return {
            mensagem: "Condutor já cadastrado com este CPF",
            data: {},
            sucesso: false,
            erro: null,
         };
      ...
      return {
         mensagem: "Erro ao cadastrar condutor",
         data: {},
         sucesso: false,
         erro: e.message,
      };
...
Parse.Cloud.define("pega-condutor-por-cpf", async (req) => {
   ...
   try {
      const resultCondutor = await queryCondutor
         .equalTo("cpf", req.params.cpf)
         .find({ useMasterKey: true });
      const retorno = await resultCondutor.map(function (p) {
         p = p.toJSON();
         return {
            id: p.objectId,
            cpf: p.cpf,
      ...
      return {
         mensagem: "Condutor carregado com sucesso",
         data: retorno,
         sucesso: true,
         erro: null,
      };
