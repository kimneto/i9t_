...
Parse.Cloud.define("cria-condutor", async (req) => {
    ...
    condutor.set("nivel", 1);
    try {
        const resultCondutor = await condutor.save(null, {useMasterKey: true});
        ...
        return {menssagem: "Condutor cadastrado com sucesso", data: p, sucesso: true, erro: null};
    } catch (e) {
        return {menssagem: "Erro ao cadastrar condutor", data: [], sucesso: false, erro: e.message};
    }
...
Parse.Cloud.define("pega-condutor-por-cpf", async (req) => {
    ...
    try {
        const resultCondutor = await queryCondutor.equalTo("cpf", req.params.cpf).find({useMasterKey: true});
        ...
        return {menssagem: "Condutor carregado com sucesso", data: retorno, sucesso: true, erro: null};
    } catch (e) {
