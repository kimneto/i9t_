...
Parse.Cloud.define("finaliza-fct", async (req) => {
   try {
      const Fct = Parse.Object.extend("Fct");
      const fct = await new Parse.Query(Fct).get(req.params.fctId);
      fct.set("defeitosVerificados", req.params.defeitos);
      fct.set("novidadesVerificadas", req.params.novidades);
      fct.set("concluido", true);
      const resposta = await fct.save(null, {
         useMasterKey: true,
      });
      console.log(resposta);
      return {
         menssagem: "Fct atualizado com sucesso",
         data: resposta,
         sucesso: true,
         erro: null,
      };
      ...
      console.log(e);
      return {
         menssagem: "Erro ao atualizar fct",
         data: {},
         sucesso: false,
         erro: e.message,
      };
