...
Parse.Cloud.define("pega-veiculos-por-status", async (req) => {
    ...
    try {
        const resultVeiculos = await queryVeiculos.equalTo("statusVeiculo", req.params.statusVeiculo).find({useMasterKey: true});
        const retorno = resultVeiculos.map(function (p) {
            p = p.toJSON();
            return {
                id: p.objectId,
        ...
        return {
            menssagem: "Lista de veículos status " + req.params.statusVeiculo + " carregada com sucesso",
            data: retorno,
            sucesso: true,
            erro: null
        };
